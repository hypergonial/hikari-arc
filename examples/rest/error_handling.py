...
async def error_command_func(
    ctx: arc.RESTContext, recoverable: arc.Option[bool, arc.BoolParams("Is the error recoverable?")] = True
) -> None:
    if recoverable:
...
async def client_error_handler(ctx: arc.RESTContext, exc: Exception) -> None:
