...
@client.include
@arc.slash_command(name="options", description="All the options!")
async def all_the_options(
    ctx: arc.RESTContext,
    # ints and floats support min and max values
    number: arc.Option[int, arc.IntParams(description="A whole number.", max=10, min=0)],
    # If you set choices, those will be the only valid values for the option
    floating: arc.Option[float, arc.FloatParams(description="A float.", choices=[1.0, 1.5, 2.0])],
    # Text options can have a max length
    text: arc.Option[str, arc.StrParams(description="Some text.", max_length=50)],
    # You can also use some common Discord types as options, these will be resolved by Discord
    user: arc.Option[hikari.User, arc.UserParams(description="A user.")],
    # To make an option not required, set a default value for it
    boolean: arc.Option[bool, arc.BoolParams(description="A boolean.")] = True,
    ...
    channel: arc.Option[hikari.TextableGuildChannel | None, arc.ChannelParams(description="A channel.")] = None,
) -> None:
    await ctx.respond(
        f"Number: `{number}`"
        f"\nFloating: `{floating}`"
